param (
    ...
    [string]$HfToken,
    [string]$GpuArch = "gfx90a"  # New parameter for AMD GPU architecture
)
...
if ($GpuArch) { Write-Host "GPU Architecture: $GpuArch" }  # New log line for GPU architecture
...
$command = "export PYTORCH_HIP_ALLOC_CONF='garbage_collection_threshold:0.8,max_split_size_mb:512' && "
$command += "PYTORCH_ROCM_ARCH=$GpuArch source /opt/conda/bin/activate kolo_env && "
$command += "tune run lora_finetune_single_device --config $configValue"
